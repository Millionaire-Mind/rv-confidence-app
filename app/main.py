...
def load_json_file(path: Path, default: Any) -> Any:
    try:
        ...
        # Fail safe: if corrupted, return default rather than crashing the whole API
        return default
    ...
    return {
        "tools": [
            ...
            },
            {
                ...
                "purpose": "Flags clarity risks and cost drivers in a quote. Does not accuse dealers.",
                "read_only": True,
                "open_world": True,
            },
            {
...
def manufacturer_intelligence(req: ManufacturerIntelligenceRequest) -> ToolEnvelope:
    warranties = load_json_file(MANUFACTURER_WARRANTIES_PATH, {})
    verified_construction = load_json_file(VERIFIED_CONSTRUCTION_PATH, {})
    ...
    if focus in ("warranty", "full"):
        entry = None
        # warranties stored with original capitalization keys, so match case-insensitively
    ...
    if focus in ("construction", "full"):
        ...
        # Find verified model entry if manufacturer+model provided
        if req.model:
            model_key = f"{m_key}:{norm_key(req.model)}"
            # verified_construction stored as dict of keys -> entry
            entry = None
            for k, v in verified_construction.items():
                if norm_key(k) == model_key:
                    entry = v
                    break
            if entry and entry.get("source_url"):
                verified_block = {
    ...
    # Future placeholder: quality_overview
    ...
    return ToolEnvelope(tool="manufacturer_intelligence", input=req.model_dump(), output=output)
...
def rv_compare(req: RVCompareRequest) -> ToolEnvelope:
    rv_models = load_json_file(RV_MODELS_PATH, {})
    ...
    def find_model(rv: RVIdentity) -> Optional[Dict[str, Any]]:
        # Store keys as manufacturer:model:year:trim (case-insensitive)
        key_parts = [norm_key(rv.manufacturer), norm_key(rv.model)]
        if rv.year is not None:
            key_parts.append(str(rv.year))
        if rv.trim:
            key_parts.append(norm_key(rv.trim))
        key = ":".join(key_parts)

        # direct hit
        if key in {norm_key(k): k for k in rv_models.keys()}:
            # map normalized -> original
            for k in rv_models.keys():
                if norm_key(k) == key:
                    return rv_models[k]
        return None

    a_entry = find_model(req.rv_a)
    b_entry = find_model(req.rv_b)

    def status_for(entry: Optional[Dict[str, Any]]) -> str:
        if not entry:
            return "not_found"
        src = entry.get("source_url")
        return "found" if src else "found"  # found but may be unverified per status below

    comparison: Dict[str, Any] = {"specs": {}}
    unknowns: List[str] = []

    def pack_specs(entry: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        if not entry:
            return {}
        source_url = entry.get("source_url")
        last_verified = entry.get("last_verified")
        ...
        unknowns.append("Specs not verified (missing or placeholder source_url).")
        return {
            "status": "unverified",
            "source_url": None,
            "last_verified": None,
    ...
    comparison["specs"]["rv_a"] = pack_specs(a_entry)
    comparison["specs"]["rv_b"] = pack_specs(b_entry)
    comparison["specs"]["note"] = "Specs are factual only when the status is verified and a source_url is provided."

    # Compute key differences only when both are verified
    def is_verified(block: Dict[str, Any]) -> bool:
        return block.get("status") == "verified" and bool(block.get("source_url"))

    key_diffs = {"status": "not_computed", "differences": [], "same_on_both": [], "note": None}
    if is_verified(comparison["specs"]["rv_a"]) and is_verified(comparison["specs"]["rv_b"]):
        a_specs = comparison["specs"]["rv_a"]["specs"]
        b_specs = comparison["specs"]["rv_b"]["specs"]
        ...
        for field, label in fields:
            if field in a_specs and field in b_specs:
                if a_specs[field] != b_specs[field]:
                    diffs.append({"field": field, "label": label, "rv_a": a_specs[field], "rv_b": b_specs[field]})
                else:
                    same.append({"field": field, "label": label, "value": a_specs[field]})
        ...
        comparison["key_differences"] = key_diffs

    output = {
        "comparison_intent": req.focus or "specs",
        "rv_a": {"input": req.rv_a.model_dump(), "data_status": status_for(a_entry)},
        "rv_b": {"input": req.rv_b.model_dump(), "data_status": status_for(b_entry)},
        "trust_disclosures": trust_disclosures,
        "unknowns": unknowns,
    ...
    return ToolEnvelope(tool="rv_compare", input=req.model_dump(), output=output)
...
def cost_depreciation_estimate(req: CostDepreciationRequest) -> ToolEnvelope:
    model = load_json_file(DEPRECIATION_MODEL_PATH, {})

    category_used = req.rv_category or "motorhome"
    category_source = "input" if req.rv_category else "default"

    # Basic baseline ranges (fallback if file missing)
    defaults = {
        "motorhome": {"y1": [0.12, 0.22], "y3": [0.25, 0.40], "y5": [0.35, 0.55]},
        "towable": {"y1": [0.10, 0.20], "y3": [0.22, 0.36], "y5": [0.32, 0.50]},
    }

    curves = model.get("curves", {}) if isinstance(model, dict) else {}
    curve = curves.get(category_used, None)

    if not curve:
        curve = defaults.get(category_used, defaults["motorhome"])

    y1 = curve.get("year_1_loss_pct_range", curve.get("y1", defaults["motorhome"]["y1"]))
    y3 = curve.get("year_3_total_loss_pct_range", curve.get("y3", defaults["motorhome"]["y3"]))
    y5 = curve.get("year_5_total_loss_pct_range", curve.get("y5", defaults["motorhome"]["y5"]))
    ...
    output = {
        ...
        "model_meta": {
            "model_version": model.get("model_version", "v1") if isinstance(model, dict) else "v1",
            "last_updated": model.get("last_updated", datetime.now().date().isoformat()) if isinstance(model, dict) else datetime.now().date().isoformat(),
        },
        "applied_adjustments": [],
        "disclosures": [
        ...
        "what_would_make_this_more_precise": [
        ...
        "depreciation_estimate": {
            ...
            if not dollar_ranges
            else "Dollar ranges computed from your purchase_price_usd input.",
        },
    }

    return ToolEnvelope(tool="cost_depreciation_estimate", input=req.model_dump(), output=output)
...
def deal_risk_scan(req: DealRiskScanRequest) -> ToolEnvelope:
    fees = req.fees or []
    ...
    flags = []
    # Simple “clarity risk” heuristics (NOT accusations)
    for f in fees:
        name_l = f.name.lower()
        if f.category == "addon" and f.disclosed_as_optional is None:
            flags.append(
                {
                    "type": "clarity_risk",
                    "severity": "medium",
                    "title": "Addon not clearly marked optional",
                    "detail": f"'{f.name}' is categorized as an add-on but optionality is not clear. Ask to remove all optional items to get a clean baseline.",
                }
            )
            ...
            flags.append(
                {
                    ...
                    "detail": "Doc fees are common but vary. Confirm whether it’s capped by your state and whether it’s negotiable.",
                }
            )

    checkpoint = {
        "suggested_next_move": "pause_and_clarify" if clarifying_questions else "review_and_compare",
        "reason": "Key details are missing. Getting a clean itemized breakdown reduces surprise and makes comparisons fair."
        if clarifying_questions
        else "You have enough detail to compare out-the-door totals across options.",
    }

    output = {
        "status": "ok",
        "data_confidence": {
            "level": "stub" if clarifying_questions else "partial",
            "meaning": "This scan flags clarity risks and cost drivers based on provided inputs. It does not prove wrongdoing.",
            "can_be_used_as_fact": False,
        },
        "summary": {
    ...
    return ToolEnvelope(tool="deal_risk_scan", input=req.model_dump(), output=output)
...
def maintenance_repair_triage(req: MaintenanceRepairTriageRequest) -> ToolEnvelope:
    ...
    if stop_triggers:
        ...
        return ToolEnvelope(tool="maintenance_repair_triage", input=req.model_dump(), output=output)

    # Low-risk guidance (kept general; not step-by-step repairs)
    output = {
        "status": "ok",
        "safety_level": "caution",
        "reason": "No immediate high-risk red flags selected. Guidance is general and safety-first.",
        "next_steps": [
            "Identify the exact component and conditions when the symptom occurs (when parked vs moving, shore power vs battery, propane on vs off).",
            "Check for obvious issues you can observe safely (loose fittings, visible leaks, blown fuses) without disassembly.",
...
# ----------------------------
# Custom OpenAPI: inject servers for Actions
# ----------------------------
def custom_openapi():
